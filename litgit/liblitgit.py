...
def main(argv=sys.argv[1:]):
    if not argv:  # If no arguments are provided
        argparser.print_help()  # Display the help message
...
def kvlm_parse(raw, start=0, dct=None):
    if not dct:
        ...
        #call to the functions will endelessly grow the same dict:
    ...
    #If space appears before nweline, we have a keyword. Otherwise,
...
def tree_parse_one(raw, start=0):
    ...
    sha = format(int.from_bytes(rae[y+1:y+21], "big"), "040x")
...
def tag_create(repo, name, ref, create_tag_object=False):
    # get the GitObject from the object referece
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'Litgit <litgit@example.com>'
        # ...and a tag message!
        tag.kvlm[None] = b"A tag generated by litgit, which won't let you customize the message!"
...
def object_find(repo, name, fmt=None, follow=True):
    ...
    while True:
        obj = object_read(repo, sha)
        #     ^^^^^^^^^^^^^^^ < this is a bit aggressive: we're reading
...
argsp.add_argument("--litgit-type",
                   metavar="type",
                   dest="type",
                   choices=["blob", "commit", "tag", "tree"],
                   default=None,
...
class GitIndexEntry (object):
    def __init__(self, ctime=None, dev=None, ino=None,
...
def index_read(repo):
    ...
    assert version == 2, "litgit only support index file version 2"
    ...
    for i in range(0, count):
        ...
        flag_extended = (flags & 0b0011000000000000)
        # Length of the name. This is stored on 12 bits, some max
        # value is 0xFFF, 4095. Since names can occassionally go
        # beyonds that length, git treats 0xFFF as meaning at least
...
def gitignore_read(repo):
    ...
    for entry in index.entries:
        if entry.name == ".gitignore" or entry.name.edswith("/.gitignore"):
...
def check_ignore(rules, path):
    ...
    result = check_ignore_sccoped(rules.scoped, path)
    if result != None:
        return result
...
def tree_to_dict(repo, ref, prefix=""):
    ...
    for leaf in tree.items:
        ...
        # expensive: we could just open it as a file and read the first few bytes)
        is_subtree = leaf.mode.startswith(b'04')

        # Depending on the type, we either store thr path (if it's a
...
def cmd_status_head_indec(repo, index):
